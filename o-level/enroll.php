...
<head>
  ...
  <style>
    *{box-sizing:border-box;font-family:'Poppins',sans-serif;}
    ...
    .coupon-section{background:#f7f9ff;border:1px dashed #b9c5ff;border-radius:10px;padding:15px;margin-top:10px;}
    .coupon-section input{width:calc(100% - 110px);display:inline-block;margin-right:10px;}
    .coupon-section button{width:90px;background:#3a53ff;}
    #couponMsg{font-size:13px;margin-top:8px;}
...
function applyCoupon(){
  ...
  if(!coupon){ alert("Please enter a coupon code."); return; }
  ...
  .then(data=>{
    ...
    if(data.success){
      ...
      msg.innerHTML=`✅ <b>${data.message}</b><br>Coupon Applied: <b>${coupon}</b><br>Discount: ₹${data.discount}<br>New Payable Amount: ₹${data.newAmount.toFixed(2)}`;
    }else{
      msg.style.color="red";msg.innerHTML="❌ "+data.message;
    }
  })
...
function saveEnrollment(name,email,phone,address,plan,price,payment_id){
  ...
  xhr.send(`name=${name}&email=${email}&phone=${phone}&address=${address}&plan=${plan}&price=${price}&payment_id=${payment_id}&emi_mode=${emiMode}&emi_months=${emiMonths}&emi_remaining=${remainingEMI}`);
}
</script>
</body>
</html>
